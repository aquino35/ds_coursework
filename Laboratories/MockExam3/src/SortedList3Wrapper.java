...
public class SortedList3Wrapper {
    ...
    public static class SortedCircularDoublyLinkedList<E> implements SortedList<E> {
        ...
        public SortedList<E> nearestPredecessors(E e) {
            // ADD CODE HERE
            int count = 0;
            if(!this.contains(e)) return new SortedCircularDoublyLinkedList<>(comparator);
            SortedList<E> result  = new SortedCircularDoublyLinkedList<>(comparator);
            for (int index = this.firstIndex(e)-1; count < 3 && (index >= 0 &&index < this.size()); index--, count++)
