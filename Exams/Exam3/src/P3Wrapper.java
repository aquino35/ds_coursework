...
public class P3Wrapper {
    ...
    public static class BinarySearchTree<K, V> implements Map<K, V> {
         ...
         * all the key value pairs from the tree that have a key that is less than
         * a given key, which is passed as parameter. Recall the private class MapEntry from
        ...
        public ArrayList<KeyValuePair<K, V>> lessThanValues(K key) {
            // ADD CODE HERE
            // YOU CAN USE AUXILIARY METHOD
            ArrayList<KeyValuePair<K, V>> List = new ArrayList<KeyValuePair<K, V>>();
            if (this.root == null) return List;
            return lessThanValuesAux(key, root, List);
        }

        public ArrayList<KeyValuePair<K, V>> lessThanValuesAux(K key,BinaryTreeNode<MapEntry<K, V>> Node
                                                               ,ArrayList<KeyValuePair<K, V>> List) {

            if (Node == null) return List;
            if (this.keyComparator.compare(Node.getValue().getKey(), key) < 0) {
                List.add(new MapEntry<K, V>(Node.getValue().getKey(), Node.getValue().getValue()));
            }
            lessThanValuesAux(key, Node.getLeftChild(), List);
            lessThanValuesAux(key, Node.getRightChild(),List);
            return List;
