...
public class P3Wrapper {
    ...
    public static class BinarySearchTree<K, V> implements Map<K, V> {
        ...
        /*
         * Implement a member method inFullPath which determines if key value pair is located on a BST where each of its
         * ancestors have two children (i.e., left and child children). Do not count the node with the key value pair as its own ancestor. The method
         * receives as parameter the key of the key-value pair to be searched in the BST. The method
         * return true is the key-value pair lies on such path, or false otherwise. The method returns false if the element
         * key is not found on the  BST, or if the key is in the root.
         */
        public boolean inFullPath(K key) {
            // ADD YOUR CODE HERE
            if(root==null || root.getValue().getKey().equals(key)){
                return false;
            }
            return inFullPathAux(key, root);
        }

        private boolean inFullPathAux( K key,BinaryTreeNode<MapEntry<K, V>> Node) {

            int comparison = this.keyComparator.compare(key, Node.getValue().getKey());

            if(comparison >= 0 && Node.getRightChild() != null && Node.getLeftChild() != null){
                return inFullPathAux( key,Node.getRightChild());

            }
            else if (comparison <= 0 && Node.getRightChild() != null && Node.getLeftChild() != null){
                return inFullPathAux(key, Node.getLeftChild());
        ...
        public ArrayList<KeyValuePair<K, V>> inRangeValues(K key1, K key2) {
            ...
            inRangeValuesAux(key1,key2,root, L);
            return L;
        }

        public void inRangeValuesAux(K key1, K key2,BinaryTreeNode<MapEntry<K,V>> Node, ArrayList<KeyValuePair<K, V>> L ){
            if (Node == null) return;
            int comparison1 = this.keyComparator.compare(Node.getValue().getKey(),key1);
            int comparison2 = this.keyComparator.compare(Node.getValue().getKey(),key2);
            if (comparison1 >= 0 && comparison2 < 0){ L.add(Node.getValue()); }
            inRangeValuesAux( key1, key2, Node.getLeftChild(), L);
            inRangeValuesAux( key1, key2, Node.getRightChild(), L);
        }
    }
